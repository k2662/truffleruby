...
module Commands
  ...
  def check_cext_files
    run_ruby('tool/generate-cext-trampoline.rb')
    diff = sh 'git', 'diff', 'src/main/c/cext-trampoline/trampoline.c', capture: :out
    unless diff.empty?
      STDERR.puts 'DIFF:'
      STDERR.puts diff
      abort "src/main/c/cext-trampoline/trampoline.c must be regenerated by 'tool/generate-cext-trampoline.rb'"
    end
  end
